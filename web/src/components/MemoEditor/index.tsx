...
// Subscribes to the editor's content slice in isolation so typing doesn't
// re-render MemoEditorImpl's shell (see the comment on `useEditorSelector`
// usage below for why content isn't read there directly).
const ContentChangeBridge = ({ onContentChange }: { onContentChange: (content: string) => void }) => {
...
const MemoEditorImpl = forwardRef<EditorController, MemoEditorProps>(
  (
    ...
    useImperativeHandle(forwardedRef, () => editorRef.current as EditorController, []);
